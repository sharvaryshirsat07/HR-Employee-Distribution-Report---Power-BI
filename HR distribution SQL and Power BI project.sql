...
UPDATE hr
SET termdate = IF(termdate != '', DATE_FORMAT(STR_TO_DATE(REPLACE(termdate, ' UTC', ''), '%Y-%m-%d %H:%i:%s'), '%Y-%m-%d'), NULL)
...
SELECT
    ...
    count(*) AS count
FROM hr
WHERE age>=18 GROUP BY age_group
ORDER BY age_group;
...
SELECT
    ...
    count(*) AS count
FROM hr
WHERE age>=18 GROUP BY age_group, gender
